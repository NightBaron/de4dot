...
namespace de4dot.code.deobfuscators.ConfuserEx
{
    public class AntitamperCfx
    {
        ...
        private static ModuleDefMD modulez;
        ...
        public  void antitamperfind()
        {
            var moduleType = blocks.DotNetUtils.GetModuleType(modulez);
            if (moduleType == null)
            {
                return;
            }
            foreach (var method in moduleType.Methods)
            {
                ...
                try
                {
                    if (IsTampered(modulez) == true)
                    {
                        Logger.v("[!] Anti Tamper Detected");

                        byte[] rawbytes = null;

                        var htdgfd = (modulez).MetaData.PEImage.CreateFullStream();

                        rawbytes = htdgfd.ReadBytes((int)htdgfd.Length);
                        try
                        {
                            modulez = UnAntiTamper(modulez, rawbytes);
                            Logger.v("[!] Anti Tamper Removed Successfully");
                        }
                        catch
                        {
                            Logger.v("[!] Anti Tamper Failed To Remove");
                        }
                ...
                {
                    Logger.v("An error in anti tamper remover happened");
                }
            }
        }

        public AntitamperCfx(ModuleDefMD module, ISimpleDeobfuscator deobfuscator)
        {
            modulez = module;
